...
from src.deploy import rollout_from_manifest
...
def cli():
    ...
    if args.command == "init":
        ...
        result = rollout_from_manifest(args.manifest)
        if not result.ready_for_traffic:
            print(f"Rollout blocked: {result.reason}", file=sys.stderr)
            sys.exit(2)
        print(
            f"Deployment ready: {result.active_version} serving after "
